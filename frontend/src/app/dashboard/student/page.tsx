              ...
              <div className="flex items-center justify-between relative">
                <div>
                  ...
                  <p className="text-3xl font-bold text-white">
                    {notifications.filter((n:any)=>!n.read && !n.is_read).length || notifications.length}
                  </p>
                  <p className="text-xs text-gray-400 mt-1">
                    {notifications.filter((n:any)=>!n.read && !n.is_read).length > 0 ? 'New messages' : 'All caught up'}
                  </p>
                </div>
                <div className="bg-blue-500/30 p-4 rounded-xl backdrop-blur-sm">
                  <Bell className="w-7 h-7 text-blue-300" />
                  {notifications.filter((n:any)=>!n.read && !n.is_read).length > 0 && (
...
{(upcoming || []).map((it:any, idx:number) => {
            ...
            <div className="space-y-8">
              ...
              {/* Notifications */}
              <div className="bg-gradient-to-br from-black/50 to-gray-900/50 border border-white/20 rounded-xl p-6 shadow-xl">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-semibold text-white flex items-center">
                  ...
                  {notifications.length > 0 && (
                    <span className="bg-blue-500 text-white text-xs px-3 py-1 rounded-full">
                      {notifications.filter((n:any) => !n.read && !n.is_read).length || notifications.length} new
                    </span>
                  )}
                </div>
                <div className="space-y-3">
                  {notifications.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <div className="text-xs text-gray-500 mt-1">You&apos;ll see important updates here</div>
                    </div>
                  ) : (
                    (notifications || []).slice(0, 5).map((n:any, idx:number) => {
                      const nType = n.type || n.level || 'info';
                      const isUnread = !n.read && !n.is_read;
                      const bgClass = isUnread ? 'bg-blue-500/10 border-blue-400/30' : 'bg-white/5 border-white/10';
                      const dotClass = nType === 'urgent' ? 'bg-red-400' : (nType === 'warning' ? 'bg-yellow-400' : 'bg-blue-400');
                      
                      return (
                        <div key={n.id || idx} className={`${bgClass} rounded-lg p-4 border transition-all duration-200 hover:bg-white/10 ${isUnread ? 'shadow-md' : ''}`}>
                          <div className="flex items-start">
                            <div className={`w-3 h-3 rounded-full mt-1.5 mr-4 flex-shrink-0 ${dotClass} ${isUnread ? 'animate-pulse' : ''}`} />
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between mb-1">
                                <h4 className={`text-sm font-semibold ${isUnread ? 'text-white' : 'text-gray-300'}`}>
                                  {n.title || n.header || 'Notification'}
                                </h4>
                                {isUnread && (
                                  <span className="bg-blue-500 text-white text-xs px-2 py-1 rounded-full ml-2">New</span>
                                )}
                              </div>
                              <p className={`text-sm ${isUnread ? 'text-gray-200' : 'text-gray-400'} leading-relaxed`}>
                                {n.message || n.text || n.description}
                              </p>
                              {(n.time || n.created_at) && (
