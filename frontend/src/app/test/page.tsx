...
export default function TestPage() {
  ...
  const testCORS = async () => {
    ...
    try {
      ...
      setResult(`Server Connection Test Successful: ${JSON.stringify(data, null, 2)}`);
    } catch (error) {
      setResult(`Server Connection Failed: ${error}. Make sure backend server is running on port 8000.`);
    }
    setLoading(false);
  };
  ...
  const testLogin = async () => {
    ...
    try {
      const response = await authAPI.login({
        email: 'dr.amit.kumar@mpstme.edu.in',
        password: 'testpassword123'
      });
      setResult(`Faculty Login Test Successful: ${JSON.stringify(response, null, 2)}`);
    } catch (error: unknown) {
      const errorMessage = error && typeof error === 'object' && 'response' in error
        ? (error as { response?: { data?: { detail?: string } } }).response?.data?.detail
        : error instanceof Error ? error.message : 'Unknown error';
      setResult(`Faculty Login Test Failed: ${errorMessage}`);
    }
    setLoading(false);
  };
  ...
  const testStudentLogin = async () => {
    ...
    try {
      const response = await authAPI.login({
        email: 'arjun.sharma@mpstme.edu.in',
        password: 'testpassword123'
      });
      setResult(`Student Login Test Successful: ${JSON.stringify(response, null, 2)}`);
    } catch (error: unknown) {
      const errorMessage = error && typeof error === 'object' && 'response' in error
        ? (error as { response?: { data?: { detail?: string } } }).response?.data?.detail
        : error instanceof Error ? error.message : 'Unknown error';
      setResult(`Student Login Test Failed: ${errorMessage}`);
    }
    setLoading(false);
  };

  const getActualUsers = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:8000/auth/get-sample-users', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const data = await response.json();
      setResult(`Sample Users from Database: ${JSON.stringify(data, null, 2)}`);
    } catch (error) {
      setResult(`Get Users Failed: ${error}`);
    }
    setLoading(false);
  };
